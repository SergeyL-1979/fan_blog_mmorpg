...
# from mmorpg.models import Ad, Respond
...
#     instance = Respond.objects.get(pk=pk)
#     if created:
#         usr = MyUser.objects.get(pk=usr_pk)
#         html_content = render_to_string(
#             'mmorpg/subs_email.html',
#             {
#                 'comment': instance,
#                 'usr': usr,
#                 'full_url': full_url,
...
#             body=f'Hello, {usr.first_name} {usr.last_name}. New comment on your post: ' + instance.post_title,
...
#             body=f'Hello, {usr.first_name} {usr.last_name}. Your comment to post: ' + instance.post_title + ' was be approved!!',
#             # это то же, что и message
#             from_email=settings.DEFAULT_FROM_EMAIL,
#             to=[f'{usr.email}'],  # это то же, что и recipients_list
#         )
...
#     full_url = ''.join(['http://', get_current_site(None).domain, ':8000'])
#
#     list_of_posts = Ad.objects.filter(date_posted__range=(start_date, end_date))
#     if len(list_of_posts) > 0:
#         for u in MyUser.objects.filter(need_mailing_news=True):
#             html_content = render_to_string(
#                 'mmorpg/subs_email_each_week.html',
#                 {
#                     'news': list_of_posts,
#                     'user': u,
#                     'full_url': full_url,
